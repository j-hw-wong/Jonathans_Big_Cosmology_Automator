...
def execute_poisson_sampling():
    ...
    obs_ids = np.where((mask_map == 1))[0]
    masked_ids = np.where((mask_map == 0))[0]
    numb_dens_maps = []
    corr_pixels = []
    ...
    for k in range(len(z_sample)):
        ...
        #good_pixels_ids = np.where((raw_field_vals >= -1))[0]
        #corrected_pixels_map = np.zeros(npix)
        #corrected_pixels_map = corrected_pixels_map - 1
        #weighting_map = corrected_pixels_map/raw_field_vals
        #weighting_map[good_pixels_ids] = 1
        # multiply the above by the raw map to get the corrected map ?
        ...
        if min(raw_field_vals[np.where(mask_map == 1)]) < -1:
            weighting = 1/abs(min(raw_field_vals[np.where(mask_map == 1)])-(1e-8))
            norm_field_vals = norm_field_vals*weighting

        else:
            norm_field_vals = norm_field_vals
            weighting = 1
        ...
        numb_dens_map_mod = poisson_sample_masked_map(expected_numb_dens, mask_map, npix)
        numb_dens_map = numb_dens_map_mod * ((raw_field_vals+1)/(norm_field_vals+1))
        #print(numb_dens_map[obs_ids])
        numb_dens_map = np.around(numb_dens_map)
        numb_dens_map[masked_ids] = hp.UNSEEN
        #print(numb_dens_map[obs_ids])
        numb_dens_map[obs_ids] = numb_dens_map[obs_ids].astype(int)

        #zslice_numb_dens.append(nbar)
        #zslice_weightings.append(weighting)
        ...
        for n in range(len(numb_dens_map)):
            ...
            if pixel_occupation != hp.UNSEEN:
                if pixel_occupation >= 0:
                    ids.extend([n] * int(numb_dens_map[n]))
                    ids_weightings = np.zeros(int(pixel_occupation))
                    ids_weightings = ids_weightings + 1
                    weightings.append(ids_weightings)
                if pixel_occupation < 0:
                    ids.extend([n] * abs(int(numb_dens_map[n])))
                    ids_weightings = np.zeros(abs(int(pixel_occupation)))
                    ids_weightings = ids_weightings - 1
                    weightings.append(ids_weightings)

        weightings = np.concatenate(weightings, axis=0)
        ids = np.asarray(ids)
        #print(ids)
        #print(weightings)
        ids = ids.astype(int)

        #gal_weightings = np.zeros(len(ids))
        #gal_weightings = gal_weightings + weighting
        final_weightings.append(weightings)
    ...
    final_weightings = [item for sublist in final_weightings for item in sublist]

    final_positions = np.asarray(final_positions)
    final_redshifts = np.asarray(final_redshifts)
    final_weightings = np.asarray(final_weightings)
    ...
    cat.create_dataset("Galaxy_Weighting", data=final_weightings)
    ...
    '''
    weightings_dir = cats_dir + 'weightings/'
    if not os.path.exists(weightings_dir):
        os.makedirs(weightings_dir)

    np.savetxt(weightings_dir + 'weightings_{}.txt'.format(iter_no),
               np.transpose(np.array([z_sample, np.asarray(zslice_numb_dens), np.asarray(zslice_weightings)])))
    '''
