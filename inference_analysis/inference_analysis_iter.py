...
for threshold in thresholds:
    ...
    cov_fromsim_path = save_dir + 'cov_fromsim_threshold_{}/'.format(threshold)
    ...
    if obs_type == '3X2PT':
        print(threshold, cov_fromsim_path)
    ...
    os.rename(inference_analysis_output_dir + 'like_lmaxlike%s_10bp.txt' % (output_lmax),
              inference_analysis_output_dir+'like_lmaxlike%s_10bp_covfromsim%s.txt' % (output_lmax, threshold))

    posterior.cl_post(
        log_like_filemask=inference_analysis_output_dir+'like_lmaxlike%s_10bp_covfromsim%s.txt' % (output_lmax, threshold),
        ...
        obs_type=obs_type,
        plot_save_path=inference_analysis_output_dir+'contours_l{}-{}_{}_threshold_{}.png'.format(output_lmin, output_lmax, obs_type, threshold)
    )
